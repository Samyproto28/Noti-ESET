...
function main() {
  colorLog('🚀 Iniciando ejecución de pruebas TDD para Noti-ESET Backend', 'blue');
  
  // Ejecutar pruebas unitarias de servicios
  const serviceUnitResults = runCommand(
    'npm test -- --testPathPattern=unit/services/',
    'Ejecutando pruebas unitarias de servicios'
  );
  
  if (serviceUnitResults.success) {
    const serviceResults = checkTestResults(serviceUnitResults.output);
    const serviceTestsDetectingErrors = analyzeTestResults(serviceResults, 'Unitarias de Servicios');
    
    // Ejecutar pruebas de integración de API
    const apiIntegrationResults = runCommand(
      'npm test -- --testPathPattern=integration/',
      'Ejecutando pruebas de integración de API'
    );
    
    if (apiIntegrationResults.success) {
      const apiResults = checkTestResults(apiIntegrationResults.output);
      const apiTestsDetectingErrors = analyzeTestResults(apiResults, 'Integración de API');
      
      // Resumen final
      colorLog('\n📋 RESUMEN DE PRUEBAS TDD', 'blue');
      colorLog('=====================================', 'blue');
      
      if (serviceTestsDetectingErrors && apiTestsDetectingErrors) {
      ...
      colorLog('1. Corregir las funciones de validación en forumService.js', 'cyan');
      colorLog('2. Mejorar la sanitización de entrada para prevenir XSS', 'cyan');
      colorLog('3. Implementar manejo adecuado de errores en authController.js', 'cyan');
      colorLog('4. Corregir el middleware optionalAuthMiddleware', 'cyan');
      colorLog('5. Implementar rate limiting en las rutas del API', 'cyan');
      colorLog('6. Configurar CORS adecuadamente', 'cyan');
      colorLog('7. Ejecutar las pruebas nuevamente para verificar las correcciones', 'cyan');
      
    } else {
      colorLog('❌ Error al ejecutar pruebas de integración de API', 'red');
    }
  } else {
    colorLog('❌ Error al ejecutar pruebas unitarias de servicios', 'red');
  }
}
